...
const Explosion = ({ ...props }) => {
  // Using a deterministic approach for particle generation
  const generateParticles = () => {
    const particles = [];
    for (let i = 0; i < 20; i++) {
      // Using a deterministic pattern based on index
      const angle = (i / 20) * Math.PI * 2;
      const distance = 10 + (i % 5) * 8;
      particles.push({
        id: i,
        directionX: Math.cos(angle) * distance,
        directionY: Math.sin(angle) * distance - 20, // Bias upward
      });
    }
    return particles;
  };

  const spans = generateParticles();
    ...
    <div {...props} className={cn("absolute z-50 h-2 w-2", props.className)}>
      ...
      {spans.map((span) => (
        <motion.span
          ...
          transition={{ duration: 0.5 + (span.id % 10) / 10, ease: "easeOut" }}
